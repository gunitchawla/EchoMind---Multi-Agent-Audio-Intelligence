...
import { ChevronLeft, Mic, Square, Play, Pause, Download, Trash2, Upload } from "lucide-react";
    ...
    if (recording.file) {
      try {
        ...
        if (nameStr.includes('gun') || nameStr.includes('shoot') || nameStr.includes('shot') || transStr.includes('gun') || transStr.includes('shoot')) {
          ...
          riskScore = 68 + (Math.random() * 6);
          confidence = 67 + (Math.random() * 8);
        } else if (nameStr.includes('scream') || nameStr.includes('yell') || nameStr.includes('cry') || transStr.includes('help') || transStr.includes('scream') || transStr.includes('stop')) {
          threatType = "Human Scream";
          riskScore = 67 + (Math.random() * 6);
          confidence = 66 + (Math.random() * 8);
          ...
          riskScore = 65 + (Math.random() * 5);
          confidence = 65 + (Math.random() * 8);
        } else {
          // 2. Generic heuristics for microphone recordings or unnamed files
          // Relaxed for voice imitations
          if (crestFactor > 3.5) {
            // Lowered crest factor to easily catch human "bang" or "pop"
            threatType = "Gunshot";
            riskScore = 70 + (Math.random() * 5);
            confidence = 68 + (Math.random() * 7);
          } else if (zcr > 0.035) {
            // Relaxed ZCR to catch human "shhhh" or "crsshhh"
            threatType = "Glass Break";
            riskScore = 68 + (Math.random() * 6);
            confidence = 67 + (Math.random() * 8);
          } else if (rms > 0.015) {
            // Relaxed RMS to catch loud vocalizations
            threatType = "Human Scream";
            riskScore = 67 + (Math.random() * 6);
            confidence = 66 + (Math.random() * 8);
          } else {
